...
#banner {
	background: url('<%=request.getContextPath()%>/images/banner.jpg')
		no-repeat;
	width: 100%;
	height: 180px;
	position: relative;
...
<div id="warp">
	...
	<center>
		<div>
			...
			<table>
				...
				<s:iterator value="list" status="st" var="row">
					<tr>
						<td><img
							src="<%=request.getContextPath()%>/images/folder_new.gif"
							style="display: inline-block; margin: 4px 5px 0px 0px;" /></td>
						<td style="text-align: left;">
						<s:url var="url_viewDetail" action="post_viewDetail" >
							<s:param name="pid"><s:property value="#row.id"/></s:param>
						</s:url>
						<s:a href='%{url_viewDetail}'><s:property value="name" /></s:a></td>
						<td><s:property value="#row.getStudent().getNickName()" /></td>
						<td><s:property value="#row.getReplies().size()" /> / <s:property
								value="count" /></td>

						<td>
							<!-- <s:property value="publishTime" /> --> <s:date
								format="yyyy-MM-dd hh:mm:ss" name="publishTime" />
						</td>
					</tr>
				</s:iterator>
			</table>
	...
	<s:url var="url_preparePost" action="/bbs/post_preparePost.action" >
		<s:param name="bid"><s:property value="bid"/></s:param>
	</s:url>
		<!-- struts标签才能使用%{} -->
		<s:a href='%{url_preparePost}'><img src="<%=request.getContextPath()%>/images/pn_post.png"
